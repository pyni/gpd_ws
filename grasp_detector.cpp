  ...
  const std::vector<double>& workspace)
{
  ...
  for (int i = 0; i < hand_set_list.size(); i++)
  {
	...
	Eigen::Vector3d right_topcal =hands[j].getApproach();
         double cosangle;
         cosangle= right_topcal(2) ;
        if (aperture >= min_aperture_ && aperture <= max_aperture_ // make sure the object fits into the hand
          ...
          && z.minCoeff() >= workspace[4] && z.maxCoeff() <= workspace[5] &&   ((cosangle>0.75)&&(z.maxCoeff()<0.62))||((cosangle>0.98)&&(z.maxCoeff()>=0.62))    ) // avoid grasping outside the z-workspace
        {

  //std::cout<<"jjjjjjjjjjjjjjjjjjjjjjjjjj:"<<cosangle<<std::endl;
